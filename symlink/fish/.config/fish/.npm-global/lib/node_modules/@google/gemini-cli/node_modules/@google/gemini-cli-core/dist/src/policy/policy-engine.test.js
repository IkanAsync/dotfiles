    ...
    describe('checkHook', () => {
        ...
        it('should run hook checkers and deny on DENY decision', async () => {
            const hookCheckers = [
                {
                    eventName: 'BeforeTool',
                    checker: { type: 'external', name: 'test-hook-checker' },
                },
            ];
            ...
            expect(decision).toBe(PolicyDecision.DENY);
            expect(mockCheckerRunner.runChecker).toHaveBeenCalledWith(expect.objectContaining({ name: 'hook:BeforeTool' }), expect.objectContaining({ name: 'test-hook-checker' }));
        });
        ...
        it('should return DENY for ASK_USER in non-interactive mode', async () => {
            ...
            engine = new PolicyEngine({ hookCheckers, nonInteractive: true }, mockCheckerRunner);
            vi.mocked(mockCheckerRunner.runChecker).mockResolvedValue({
                decision: SafetyCheckDecision.ASK_USER,
                reason: 'Needs confirmation',
            });
        ...
        it('should match hook checkers by eventName', async () => {
            const hookCheckers = [
                {
                    eventName: 'AfterTool',
                    checker: { type: 'external', name: 'after-tool-checker' },
                },
                {
                    eventName: 'BeforeTool',
                    checker: { type: 'external', name: 'before-tool-checker' },
                },
            ];
            engine = new PolicyEngine({ hookCheckers }, mockCheckerRunner);
            vi.mocked(mockCheckerRunner.runChecker).mockResolvedValue({
            ...
            expect(mockCheckerRunner.runChecker).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ name: 'before-tool-checker' }));
            expect(mockCheckerRunner.runChecker).not.toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ name: 'after-tool-checker' }));
        });
        it('should match hook checkers by hookSource', async () => {
            const hookCheckers = [
                {
                    hookSource: 'project',
                    checker: { type: 'external', name: 'project-checker' },
                },
                {
                    hookSource: 'user',
                    checker: { type: 'external', name: 'user-checker' },
                },
            ];
            engine = new PolicyEngine({ hookCheckers }, mockCheckerRunner);
            vi.mocked(mockCheckerRunner.runChecker).mockResolvedValue({
            ...
            expect(mockCheckerRunner.runChecker).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ name: 'user-checker' }));
            expect(mockCheckerRunner.runChecker).not.toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ name: 'project-checker' }));
        });
        it('should deny when hook checker throws an error', async () => {
            const hookCheckers = [
                {
                    checker: { type: 'external', name: 'failing-checker' },
                },
            ];
            engine = new PolicyEngine({ hookCheckers }, mockCheckerRunner);
            vi.mocked(mockCheckerRunner.runChecker).mockRejectedValue(new Error('Checker failed'));
            const decision = await engine.checkHook({
                eventName: 'BeforeTool',
                hookSource: 'user',
            });
        ...
        it('should run hook checkers in priority order', async () => {
            const hookCheckers = [
                {
                    priority: 5,
                    checker: { type: 'external', name: 'low-priority' },
                },
                {
                    priority: 20,
                    checker: { type: 'external', name: 'high-priority' },
                },
                {
                    priority: 10,
                    checker: { type: 'external', name: 'medium-priority' },
                },
            ];
            engine = new PolicyEngine({ hookCheckers }, mockCheckerRunner);
            vi.mocked(mockCheckerRunner.runChecker).mockImplementation(async (_call, config) => {
            ...
            expect(mockCheckerRunner.runChecker).toHaveBeenCalledTimes(1);
            expect(mockCheckerRunner.runChecker).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ name: 'high-priority' }));
        });
    });

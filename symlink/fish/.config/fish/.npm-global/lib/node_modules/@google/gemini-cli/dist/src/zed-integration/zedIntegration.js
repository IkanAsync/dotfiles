 ...
 */
import { AuthType, logToolCall, convertToFunctionResponse, ToolConfirmationOutcome, clearCachedCredentialFile, isNodeError, getErrorMessage, isWithinRoot, getErrorStatus, MCPServerConfig, DiscoveredMCPTool, StreamEventType, ToolCallEvent, debugLogger, ReadManyFilesTool, getEffectiveModel, createWorkingStdio, startupProfiler, } from '@google/gemini-cli-core';
...
export async function runZedIntegration(config, settings, argv) {
    ...
    new acp.AgentSideConnection((client) => new GeminiAgent(config, settings, argv, client), stdout, stdin);
}
...
export class Session {
    ...
    async prompt(params) {
        ...
        while (nextMessage !== null) {
            ...
            const functionCalls = [];
            try {
                const model = getEffectiveModel(this.config.getModel(), this.config.getPreviewFeatures());
                const responseStream = await chat.sendMessageStream({ model }, nextMessage?.parts ?? [], promptId, pendingSend.signal);
                nextMessage = null;
